<?php 
    ...
    if(isset($_POST['upd_general']))
    {
        ...
        if ($res) {
            logAuditTrail($_SESSION['admin_name'], 'UPDATE', 'Updated site settings: Title - ' . $frm_data['site_title'] . ', About - ' . $frm_data['site_about']);
        }
        echo $res;
    }
    ...
    if(isset($_POST['upd_shutdown']))
    {
        ...
        if ($res) {
            $status = $frm_data ? 'enabled' : 'disabled';
            logAuditTrail($_SESSION['admin_name'], 'UPDATE', 'Updated shutdown status: ' . $status);
        }
        echo $res;
    }
    ...
    if(isset($_POST['add_member']))
    {
      ...
      if(!in_array($img_r, ['inv_img', 'inv_size', 'upd_failed'])) {
        ...
        if ($res) {
            logAuditTrail($_SESSION['admin_name'], 'ADD', 'Added team member: ' . $frm_data['name']);
        }
        echo $res;
      } else {
        echo $img_r;
    ...
    if(isset($_POST['rem_member']))
    {
     ...
     if(deleteImage($img['picture'],ABOUT_FOLDER)){
      ...
      if ($res) {
          logAuditTrail($_SESSION['admin_name'], 'DELETE', 'Deleted team member with ID: ' . $frm_data['rem_member']);
      }
      echo $res;
     }
     else{
